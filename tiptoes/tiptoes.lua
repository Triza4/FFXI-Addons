...
local wc_match = windower.wc_match
...
local actions = {
    sneak = {
        { job='DNC',         type='ja',   name='Spectral Jig',   level=25, },
        { job='WHM|RDM|SCH', type='ma',   name='Sneak',          level=20, },
        { job='NIN',         type='ma',   name='Monomi: Ichi',   level=25, },
        { job='???',         type='item', name='Silent Oil',     level=1,  },
        { job='???',         type='item', name='Evanessence',    level=1,  },
    },
    invisible = {
        { job='DNC',         type='ja',   name='Spectral Jig',   level=25, },
        { job='WHM|RDM|SCH', type='ma',   name='Invisible',      level=25, },
        { job='NIN',         type='ma',   name='Tonko: Ni',      level=9,  },
        { job='NIN',         type='ma',   name='Tonko: Ichi',    level=9,  },
        { job='???',         type='item', name='Prism Powder',   level=1,  },
        { job='???',         type='item', name='Rainbow Powder', level=1,  },
        { job='???',         type='item', name='Evanessence',    level=1,  },
    },
}
...
function action_available(action)
    ...
    elseif action.type == 'item' then
        local item_id = res.items:with('name', action.name).id
        ...
        for _, bag in pairs(bags) do
            ...
            for slot, item in ipairs(items) do
                if item.id > 0 and item.id == item_id then
                    if verbose then
                        log('found', item.id, res.bags[bag.id].en, action.name)
                    end
...
windower.register_event('addon command', function(...)
    ...
    elseif arg == 'verbose' then
        verbose = not verbose
        log('Verbose is now ' .. tostring(verbose))
        return
	...
	if next_sequence and seq >= next_sequence then
        ...
        while #queue > 0 do
            ...
            if not buffactive[action.task] then
                ...
                elseif (action.name == 'Spectral Jig' or action.name == 'Evanessence') and buffactive['Sneak'] then
